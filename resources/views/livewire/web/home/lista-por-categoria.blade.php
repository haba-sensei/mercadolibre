                ...
                <div class="container">
                    ...
                    <div class="ps-section__content">
                        <div class="ps-carousel--nav owl-slider" data-owl-auto="false" data-owl-loop="true"
                            ...
                            data-owl-item-lg="4" data-owl-item-xl="5" data-owl-duration="1000" data-owl-mousedrag="on" wire:ignore>
        @endif
    ...
    @foreach ($productosF as $prods)
        @if ($indice == $prods->category->id)
            <div class="ps-product" >
                <div class="ps-product__thumbnail"><a href="{{ route('web.products.show', $prods) }}"><img
                            src="{{ Storage::url($prods->image->url) }}" alt=""></a>
                    <div class="ps-product__badge {{ $prods->stock > 1 ? '' : 'out-stock' }}">
                        {{ $prods->stock > 1 ? 'En Stock' : 'Sin Stock' }}</div>
                    <ul class="ps-product__actions">
                        <li><a href="#" data-toggle="tooltip" data-placement="top" title="Add To Cart"><i
                                    class="icon-bag2"></i></a></li>
                        <li><a href="javascript:"  title="Quick View"
                                wire:click="selectItem({{ $prods->id }})">
                                <i class="icon-eye"></i>
                            </a></li>
                        <li><a href="#" data-toggle="tooltip" data-placement="top" title="Add to Whishlist"><i
                                    class="icon-heart"></i></a></li>
                        <li><a href="{{ route('web.products.show', $prods) }}" data-toggle="tooltip" data-placement="top" title="Ver mas"><i
                                    class="icon-share"></i></a></li>
                    </ul>
                </div>
                <div class="ps-product__container"><a class="ps-product__vendor"
                        href="{{ route('web.products.show', $prods) }}">{{ $prods->category->name }}</a>
                    <div class="ps-product__content"><a class="ps-product__title" href="{{ route('web.products.show', $prods) }}"> {{ $prods->name }} </a>
                        <p class="ps-product__price sale">${{ $prods->amount }} </p>
                    </div>
                    <div class="ps-product__content hover"><a class="ps-product__title" href="{{ route('web.products.show', $prods) }}">  {{ $prods->name }} </a>
                        <p class="ps-product__price sale">${{ $prods->amount }} </p>
                    </div>
                </div>
            </div>
